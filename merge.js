...
module.exports = async () => {
  ...
  const out = fs.createWriteStream(path.join(__dirname, 'etablissements_geolocalises_20180129.csv'))
  ...
  etablissements.pipe(byline()).pipe(new Transform({
    ...
    transform(item, encoding, callback) {
      if(removeHeader) {
        ...
        const geolocLine = geolocDict[arr[0]]
        const reproject = proj4(dep2epsg[arr[12]] || dep2epsg.metro, 'EPSG:4326', geolocDict[arr[0]].slice(0,2).map(c => Number(c))).map(c => c +'')
        callback(null, arr.concat(reproject, geolocLine.slice(2)))
      }
    }
  })).pipe(join(headers)).pipe(out)
